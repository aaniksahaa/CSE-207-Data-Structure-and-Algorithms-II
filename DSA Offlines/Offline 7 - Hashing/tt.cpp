...
public:
	...
	int get_aux_hash(string s)   // O(1) // Can never be zero // prime_1 must be less than table_size
	{
		...
		return prime_2 - ( salt % prime_2 );    // ensuring that aux_hash < 
	}
};
...
public:
	...
	HashTable(int table_size, int collision_resolution_method, int hash_function_type, int c1 = 7, int c2 = 11): hash_table(table_size, NULL), hash(table_size, hash_function_type)
	{
	...
	int find(string key)
	{
		...
		else if(collision_resolution_method == DOUBLE_HASHING)
		{
			...
			//cout<<"Now searching "<<endl<<key<<gap<<hash_value<<gap<<aux_hash<<br;
			...
			{
				probe_count++;
				//cout<<probe_count<<gap<<hash_table[index]->key<<br;
...
double get_unique_hash_percentage(int hash_type, int table_size,  int key_count)
{
	...
	{
		string s = generate_random_word(5,10);
...
void report_hash_function_performance(int table_size, double load_factor)
{
	int key_count = (int)((load_factor)*table_size);

	double d1 = get_unique_hash_percentage(1,table_size,key_count);
	double d2 = get_unique_hash_percentage(2,table_size,key_count);

	cout<<"Uniqueness Percentage for Hash Function 1:  "<<d1<<" %"<<endl;
	cout<<"Uniqueness Percentage for Hash Function 2:  "<<d2<<" %"<<endl;
}
...
vector<string>generate_random_unique_words(int n, int min_len, int max_len)
{
	...
	int table_size = get_nearest_prime(2*n); 
...
void solve()
{
	...
	int cnt = 10000;
	auto unique_words = generate_random_unique_words(10000,5,10);

	n = 5000;

	HashTable H(get_nearest_prime(n),CUSTOM_PROBING,1);
	int i = 1;
	for(auto word: unique_words)
	{
		//cout<<word.length()<<gap;
		if(H.insert(word,i))
		{
			//cout<<"New inserted";
		}
	...
	for(int i=0;i<10;i++)
	{
		int id = random(0,cnt-1);
		//cout<<unique_words[id]<<br;
		int value = H.find(unique_words[id]);
	}

	cout<<H.get_probe_count()<<br;
