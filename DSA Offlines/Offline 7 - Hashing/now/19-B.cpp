...
public:
	...
	void insert(string key, int value)
	{
		if(collision_resolution_method == SEPARATE_CHAINING)
		{
			...
			max_chain_length = max(max_chain_length,(int)(hash_table[index].size()));
			key_count++;
			insert_count++;
			...
			if(insert_count%100 == 0)
			{
				...
				{
					rehash(get_nearest_prime_up(1.2*table_size));
				}
			}
		}
	...
	void remove(string key)
	{
		if(collision_resolution_method == SEPARATE_CHAINING)
		{
			...
			while(it != hash_table[index].end())
			{
				if(it->key == key)
				{
					int len = hash_table[index].size();
					hash_table[index].erase(it); // The given key exists, delete it
					if(len == max_chain_length)
					{
						max_chain_length = 0;
						for(auto chain: hash_table)
						{
							max_chain_length = max(max_chain_length,(int)(chain.size()));
						}
					}
					key_count--;
					delete_count++;
					if(delete_count%100 == 0)
					{
						if(max_chain_length < 3)
						{
							rehash(get_nearest_prime_up(0.8*table_size));
						}
					}
					return;
				}
...
void solve()
{
	...
	int methods1[] = { SEPARATE_CHAINING };

	int unique_word_count = 10000;  // default

	int search_count = 1000;  // default

	// cout<<"How many unique words will be inserted:  ";
	// cin>>unique_word_count;
	// cout<<"How many random words will be searched:  ";
	// cin>>search_count;

	/*** Report Generation for HashTable ***/
	...
	for(int n: n_values)
	{
		...
		cout<<br;
		cout<<" Collision Resolution |           Hash1            |            Hash2            |"<<endl;
		for(int i=0;i<22;i++)                                            
		{
			cout<<" ";
		}
		for(int i=0;i<hyphens-22;i++)                                            
		{
			cout<<"-";
		}
		cout<<br;
		cout<<"       Method         | Collisions  |  Avg Probes  |  Collisions  |  Avg Probes  |"<<endl;
		for(int i=0;i<hyphens;i++)                                            
		{
			cout<<"-";
		}
		cout<<br;
		for(int crm: methods1)
		{
			cout<<"     ";
			if(crm == SEPARATE_CHAINING)
			{
				cout<<"Chaining       ";
			}
			else if(crm == DOUBLE_HASHING)
			{
				cout<<"Double Hashing ";
			}
			else if(crm == CUSTOM_PROBING)
			{
				cout<<"Custom Probing ";
			}
			cout<<"  |";
			for(int type = 1; type <= 2; type++)
			{
				HashTable H(table_size,crm,type);
				...
				cout<<right<<setw(width)<<H.get_collision_count()<<"   | "<<right<<setw(width)<<fixed<<setprecision(4)<<avg_probes<<"   | ";
			}
			cout<<endl;
			for(int i=0;i<hyphens;i++)
			{
	...
	double load_factors[] = { 0.5, 0.7, 0.9 };

	cout<<"Count of Unique Words = "<<unique_word_count<<br<<br;

	for(double load_factor: load_factors)
	{
		cout<<setprecision(2)<<"Load Factor: "<<load_factor<<endl<<endl;
		report_hash_function_performance(load_factor, unique_words);
		cout<<endl;
	}
