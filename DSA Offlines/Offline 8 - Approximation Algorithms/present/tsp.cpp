...
#define check_on(n, i) bool(n & (1LL << i))
#define turn_on(n, i) (n | (1LL << i))
#define turn_off(n, i) (n & ~(1LL << i))
...
public:
    ...
    vector<edge>MST_Kruskal()  // O(m*log(m))
    {
        vector<edge>EU,MST;
...
void dfs(int u, int p, vector<int>&cycle)
{
    cycle.push_back(u);
    for(int v: adj[u])
    {
...
void Metric_Approximation_TSP(DirectedGraph G)
{
    for(int i=0;i<100;i++)
    {
        adj[i].clear();
