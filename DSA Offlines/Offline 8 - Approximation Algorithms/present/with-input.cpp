...
// For now, ignoring the lower bound on edges
DirectedGraph Create_Random_Graph(int V)
{
    ...
    for(int i=1;i<=V;i++)
    {
        ...
        {
            if(j==i) continue;
            double d = get_random_double(75.0,145.0);  // taking edges in the range [a,b] where 2a > b will always suffice
            G.addEdge(i,j,d);
        }
    }
...
void dfs_on_tree(int u, int p, vector<vector<int>>&adj, Tour&T)
{
    T.add(u);
    for(int v: adj[u])
    {
...
Tour Metric_Approximation_TSP(DirectedGraph G)
{
    ...
    auto MST = G.MST_Kruskal_for_Undirected_Version();
    for(auto e: MST)
    {
